...
proc trapPlot {ivCSV bias} {
    ...
    set f [open $ivCSV w]
    close $f
    set f2 [open "figures/TrapOccupation.csv" w]

    for {set d 0.0} {$d < [expr 10.0 + 0.001]} {set d [expr $d+0.05]} {
        set f [open $ivCSV a]
...
# This block is included when the simulation starts to diverge at a specific drain bias.
# It should be used to narrow the gap between V_ds steps and lets the simulation converge where it previously diverged.
# It requires the user to know where the simulation diverges and is usually done by trial and error.
