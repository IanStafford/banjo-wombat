...
set f [open "figures/highK_rf_channel.csv" w]
close $f
...
for {set vds 0.0} {$vds < [expr $vdsMAX + 0.1]} {set vds [expr $vds+0.25]} {
    ...
    if { [expr {$vds - floor($vds/25)*25}] == 0 } {
        plot1d graph=Channel xv=0.025 ylab= "Electric Field (V/cm)" title= "Channel Field" name = "HighK Vds = $vds"
        set f [open "figures/highK_rf_channel.csv" a]
        puts $f [print1d xv=0.025]   
        close $f
    }
}
}

source rfdevice_SiN.tcl
source GaN_modelfile_masterD
...
set f [open "figures/SiN_rf_channel.csv" w]
close $f
...
for {set vds 0.0} {$vds < [expr $vdsMAX + 0.1]} {set vds [expr $vds+0.25]} {
    ...
    if { [expr {$vds - floor($vds/25)*25}] == 0 } {
        plot1d graph=Channel2 xv=0.025 ylab= "Electric Field (V/cm)" title= "Channel Field (With 50nm Nitride Layer)" name = "SiN Vds = $vds"
        set f [open "figures/SiN_rf_channel.csv" a]
        puts $f [print1d xv=0.025]   
        close $f
    }
}
