...
proc HEMT_Struct { } {
    global Gate_Length AlThick SourceGate SourceT DrainGate DrainT FP2 rightShift
    ...
    line x loc=-0.3 spac=0.025 tag=top
    line x loc=-0.2 spac=0.025 tag=topIns
    ...
    line y loc=[expr $Gtr+$rightShift] spac=0.01 tag=FPR
    line y loc=[expr $Gtr+$DrainT] spac=0.01 tag=DrainT
    line y loc=[expr $Gtr+$rightShift+$FP2] spac=0.02 tag=FP2 
    # ^^ right edge of fp
    ...
    region HighK xlo=top xhi=topIns ylo=left yhi=FPR
    region Metal xlo=top xhi=topIns ylo=FPR yhi=FP2
    ...
    #Contacts
    contact name=FP Metal xlo=[expr -0.3-$buf] xhi=[expr -0.3+$buf] ylo=[expr $Gtr+$rightShift] yhi=[expr $Gtr+$rightShift+$FP2] add depth=1.0 width=1.0
