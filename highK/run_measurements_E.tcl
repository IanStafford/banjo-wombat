proc nearHalfVolt {V {tol 0.001}} {
    set scaled [expr {$V / 0.25}]
    set frac [expr {abs($scaled - round($scaled))}]
    return [expr {$frac < ($tol / 0.25)}]
}
...
proc run_measurements_E {ivCSV peakCSV label} {
    ...
    for {set g 0.0} {$g < 1.4} {set g [expr $g+0.05]} {
        ...
        if { [nearHalfVolt $g 0.001] } {
            if {$band} {
                ...
                sel z=Qfn
                plot1d graph=Qfn yv=0.001 xmin=-0.1 xmax=0.1 title="Qfn" name="$g"                
