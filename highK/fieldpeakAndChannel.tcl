...
for {set vds 0.0} {$vds < [expr $vdsMAX + 0.1]} {set vds [expr $vds+0.25]} {
    ...
    if { [expr {$vds - floor($vds/25)*25}] == 0 } {
        plot1d graph=Channel xv=0.025 ylab= "Electric Field (V/cm)" title= "Channel Field" name = "Vds = $vds"
...
for {set vds 0.0} {$vds < [expr $vdsMAX + 0.1]} {set vds [expr $vds+0.25]} {
    ...
    if { [expr {$vds - floor($vds/25)*25}] == 0 } {
        plot1d graph=Channel2 xv=0.025 ylab= "Electric Field (V/cm)" title= "Channel Field (With 50nm Nitride Layer)" name = "Vds = $vds"
