...
proc HEMT_Struct { } {
    ...
    # Top cap of HfO2
    region HighK xlo=topInsulator xhi=topFP ylo=left yhi=right

    # Next layer including top part of field plate
    region HighK xlo=topFP xhi=topSiN ylo=left yhi=FP_Left
    region Metal xlo=topFP xhi=topSiN ylo=FP_Left yhi=FP2
    region HighK xlo=topFP xhi=topSiN ylo=FP2 yhi=right

    # Next layer including gate nitride and second part of field plate
    region HighK xlo=topSiN xhi=bottomFP ylo=left yhi=LeftGateSiN
    ...
    region HighK xlo=topSiN xhi=bottomFP ylo=FP2 yhi=right
    ...
    region pGaN xlo=topP xhi=AlGaNTop ylo=GateL yhi=GateR
    ...
    contact name=G pGaN xlo=[expr -0.051-$buf] xhi=[expr -0.047+$buf] ylo=[expr $Gtl] yhi=[expr $Gtr] add depth=1.0 width=1.0
    ...
    contact name=G current=(Hole_pGaN-Elec_pGaN) voltage supply=0.0
    ...
    sel z=-5e17*Mater(pGaN) name=pGaN_Doping
    # from the Mg in GaN paper
    #set betaMg 3.6
    #set Et_Mg "[pdbDelayDouble pGaN Eg]/2.0 + 0.17"
