...
proc trapPlot {ivCSV bias} {
    ...
    proc solvePoint {ivCSV f2 d} {
        ...
        plot1d graph=Donor xv=0.018 xmax=0.5 xmin=0.0 \
               ylab="DonorOccupation" title="Donor" name="Vds=$d" log

        sel z=[expr {"Qfn"}]
        plot1d graph=Elec xv=0.018 ylab="Qfn(eV)" title="GaN" name="Vds=$d"

        sel z=[expr {"Econd-Qfn-0.59"}]
        plot1d graph=Hole xv=0.018 xmax=0.5 xmin=0.0 \
               ylab="Econd(eV)-Qfn-Etrap" title="GaN" name="Vds=$d"
...
trapPlot "figures/acceptor_GaN_AlGaN_7.csv" 10
